...
const DbMenuItemPc: FC<{dbImgData:dbMenuType}> = ({dbImgData}) => {
    ...
    const style = {
        'backgroundColor': hovered && !selectFlg? 'rgb(244,244,244)' : (selectFlg &&  windowSizeFlg ? '#E6E6E6': '#FAFAFA'),
        'fontWeight': selectFlg ? 'bold' : 'normal',
        'display': hiddenFlg ? 'none' : 'block',
    }
    return (
        <button onClick={selectClick} onMouseEnter={() => setHovered(true)}  onMouseLeave={() => setHovered(false)} style={style} className=" h-[50px] lg:h-[60px] px-3 text-base; lg:text-lg whitespace-nowrap hover:opacity-70 ">{dbImgData.name}</button>
    );
}
